...
var parse = Hare.parser.parse;
var evalExpr = Hare.interpreter.evalExpr;
var evalStatement = Hare.interpreter.evalStatement;
var evalStatements = Hare.interpreter.evalStatements;
var lookup = Hare.interpreter.lookup;
...
suite('evalExpression', function () {
    ...
    test('array index a[0]', function () {
      ...
      assert.deepEqual(evalExpr(expr, env), 'a');
    });
    test('array index a[2][1]', function () {
      var expr = parse('a[2][1]', 'expression');
      assert.deepEqual(evalExpr(expr, env), 'd');
    });
});
